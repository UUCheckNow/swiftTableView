//
//  UUTransitioningDelegateHelper.swift
//  UUTableView
//
...
class UUTransitioningDelegateHelper: NSObject, UINavigationControllerDelegate, UIViewControllerTransitioningDelegate {
    ...
    func startLink() {
    if link == nil {
    link = CADisplayLink(target: self, selector: #selector(UUTransitioningDelegateHelper.linkUpdate))
    ...
    func transitionConfig(config: Dictionary<String, Any>) -> UUTransitionAnimation {
    let transition = UUTransitionAnimation()
    if let durationStr = config["duration"] as? String {
    if let duration = TimeInterval(durationStr) {
    transition.duration = duration
    }
    }
    if let delegate = config["delegate"] as? UUTransitionAnimationDelegate {

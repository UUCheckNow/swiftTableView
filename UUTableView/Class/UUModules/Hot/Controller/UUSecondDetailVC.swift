...
class UUSecondDetailVC: UUBaseViewController, UUSecondDetailViewDelegate {
    
    let bg = UIView()
    let detailView = UUSecondDetailView()
    let animator = UUAnimator()
    var data = [UUSecondDetailSectionModel(), UUSecondDetailSectionModel(), UUSecondDetailSectionModel(), UUSecondDetailSectionModel()]
    ...
    override func viewDidLoad() {
        ...
        setupData()
        detailView.frame = CGRect(x: kScreenWidth, y: 0, width: kScreenWidth - 60, height: kScreenHeight)
    ...
    func setupCellModel(text: String, value: inout Int) -> UUSecondDetailCellModel {
        let model = UUSecondDetailCellModel()
        model.text = text
        model.value = value
    ...
    func setupAnimator() {
        ...
        animator.setup(panGestureVC: self, transitionAction: {
            ...
            return ["duration" : "0.4", "delegate" : UUSecondDetailDismissAnimation()]
        }
    }

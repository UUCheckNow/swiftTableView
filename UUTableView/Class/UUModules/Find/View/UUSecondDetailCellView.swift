...
protocol UUSecondDetailCellViewDelegate: NSObjectProtocol {
    func cellTapped(index: Int)
}

class UUSecondDetailCellView: UIView {
    
    let bg = UIView()
    let label = UILabel()
    var model: UUSecondDetailCellModel?
    weak var delegte: UUSecondDetailCellViewDelegate?
    ...
    init() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(UUSecondDetailCellView.tapped))
    ...
    func setupData(data: Any?) {
        model = data as? UUSecondDetailCellModel
        if let m = model {
            label.text = m.text
            setupSelection()
        }
